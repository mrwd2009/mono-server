...
import useMergedState from '../../../dependents/util/src/hooks/useMergedState';
...
const Tooltip: React.FC<{ children: React.ReactElement, title: React.ReactNode }> = ({ children }) => {
  return children;
};
...
const Base = React.forwardRef((props: InternalBlockProps, ref: any) => {
  ...
  // focus edit icon after edting completed
  useUpdatedEffect(() => {
    if (!editing) {
  ...
  const copyIdRef = React.useRef<NodeJS.Timeout | null>();

  const cleanCopyId = () => {
    if (copyIdRef.current) {
      clearTimeout(copyIdRef.current);
      copyIdRef.current = null;
    }
  };
  ...
  // it's better to check cleared timer id
  ...
  React.useEffect(() => {
    ...
    // todo add isNativeEllipsis
  }, [enableEllipsis, cssEllipsis, children, cssLineClamp, isNativeEllipsis]);
  ...
  const topAriaLabel = React.useMemo(() => {
    ...
    return undefined;
    // todo add children remove unused
  }, [enableEllipsis, cssEllipsis, title, tooltipTitle, children]);
