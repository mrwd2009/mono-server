...
import { Select, Tooltip, Button, Spin } from 'antd';
...
class ConfirmableSelect extends PureComponent<Props, any> {
  ...
  filterLabelList = memoizeOne((dataSource, searchText) =>
    filter(dataSource, (listItem) => listItem.label.toLowerCase().includes(searchText.toLowerCase())),
  );
  ...
  handleChange = (value: any) => {
    const { onUpdate = identity, confirm } = this.props;
    const { inputValue } = this.state;
  ...
  handleDropdownVisible = (open: any) => {
    const { onDropdownVisibleChange = noop } = this.props;
    this.setState({
      filtering: false,
    });
   ...
   */
  handleSave = () => {
    const { onChange = noop, virtualOption } = this.props;
  ...
  getDropdownRender = (item: any) => {
    const {
      ...
      virtualOption: { list, height },
      filterLabel = false,
    } = this.props as any;
    const { inputValue, searchText, filtering } = this.state;
    if (filtering) {
  ...
  updateSelectedItem(value: any) {
    const { virtualOption } = this.props;
    if (virtualOption) {
      const { list, selectItem = false } = virtualOption;
      if (!selectItem) {
        return;
      }
  ...
  render() {
    ...
    const { inputValue, selectTooltipVisible } = this.state;
    let copyEle = null;
    ...
    return (
      <div className={`contract-confirmable-select ${className}`}>
        <div className="clearfix">
          ...
          <div className="contract-confirmable-select__content">{labelEle}</div>
        </div>
        <div className="clearfix">
          <div className="contract-confirmable-select__extra">{confirmBtns}</div>
          <div
