...
const ScrollShadow: FC<Props> = ({ className = '', style, children }) => {
  ...
  const updateShadowRef = useRef(debounce(() => {
    const {
      scrollHeight = 0,
      offsetHeight = 0,
      scrollWidth = 0,
  ...
  return (
    <ResizeObserver onResize={updateShadowRef.current}>
      <div className="scroll-shadow-container" style={style}>
        <div className={`scroll-shadow-content ${className}`} style={style} {...contentProps} >
          {content}
        </div>
