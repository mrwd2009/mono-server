  ...
  return (
    ...
    >
      ...
      <Col flex="auto">
        <Checkbox
          checked={col.visible}
          onChange={(event) => onColVisibleChange({ id: col.key, visible: event.target.checked })}
        >
          {col.title}
        </Checkbox>
      </Col>
...
const ColumnList: FC<Pick<Setting, 'cols' | 'onColPositionChange' | 'onColVisibleChange'>> = ({
  cols,
  onColPositionChange,
  onColVisibleChange,
}) => {
        ...
        {map(cols, (col) => {
          return (
            <ColumnItem
              key={col.key}
              col={col}
              onColVisibleChange={onColVisibleChange}
            />
          );
        })}
      </SortableContext>
...
const ColumnSetting: FC<{ title?: string; setting: Setting }> = ({ title = 'Setting', setting }) => {
  const popTitle = (
    ...
    >
      <Col flex="none">
        <Checkbox
          checked={setting.allChecked}
          onChange={(event) => setting.onCheckAll(event.target.checked)}
        >
          Columns Visibility
        </Checkbox>
      </Col>
  ...
  const content = (
    <ColumnList
      cols={setting.cols}
      onColPositionChange={setting.onColPositionChange}
      onColVisibleChange={setting.onColVisibleChange}
    />
  );
  return (
    <Tooltip title={title}>
