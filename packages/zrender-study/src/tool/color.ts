...
export function clampCssByte(i: number): number {
  i = Math.round(i);
  return i < 0 ? 0 : i > 255 ? 255 : i;
}

export function clampCssAngle(i: number): number {
  i = Math.round(i);
  return i < 0 ? 0 : i > 360 ? 360 : i;
}

export function clampCssFloat(f: number): number {
  return f < 0 ? 0 : f > 1 ? 1 : f;
}

export function parseCssInt(val: string | number): number {
...
export function parseCssFloat(val: string | number): number {
  let str = val as string;
  if (str.length && str.charAt(str.length - 1) === '%') {
...
export function cssHueToRgb(m1: number, m2: number, h: number): number {
  if (h < 0) {
    h += 1;
  } else if (h > 1 ) {
    h -= 1;
...
export function lerpNumber(a: number, b: number, p: number): number {
  return a + (b - a) * p;
}

export function setRgba(out: number[], r: number, g: number, b: number, a: number): number[] {
  out[0] = r;
  out[1] = g;
  out[2] = b;
  out[3] = a;
...
export function copyRgba(out: number[], a: number[]) {
  out[0] = a[0];
  out[1] = a[1];
  out[2] = a[2];
  out[3] = a[3];
...
let lastRemovedAttr: number[] = null;

export function putToCache(colorStr: string, rgbaArr: number[]) {
  if (lastRemovedAttr) {
    copyRgba(lastRemovedAttr, rgbaArr);
  }
  lastRemovedAttr = colorCache.put(colorStr, lastRemovedAttr || (rgbaArr.slice()));
}
...
export function hsla2rgba(hsla: (number | string)[], rgba?: number[]): number[] {
...
export function rgba2hsla(rgba: number[]): number[] {
  if (!rgba) {
      return;
  }
