...
function pathHasFill(style: AllStyleOption): boolean {
  const fill = (style as PathStyleProps).fill;
  return fill != null && fill !== NONE;
}

function pathHasStroke(style: AllStyleOption): boolean {
...
function roundLongFloat(val: number): number {
  // translate 0.999999999 to 1
  return mathRound(val * 1e10) / 1e10;
}

function setTransform(svgEl: SVGElement, m: matrix.Matrix) {
  if (m) {
    // I don't we need to avoid large string of matrix in original design
    // precision is more important
    attr(svgEl, 'transform', `matrix(${roundLongFloat(m[0])}, ${roundLongFloat(m[1])}, ${roundLongFloat(m[2])}, ${roundLongFloat(m[3])}, ${roundLongFloat(m[4])}, ${roundLongFloat(m[5])})`);
  }
}
...
function bindStyle(svgEl: SVGElement, style: AllStyleOption, el?: Path | TSpan | ZRImage): void {
  ...
  if (pathHasFill(style)) {
    const fill = normalizeColor((style as PathStyleProps).fill as string);
    attr(svgEl, 'fill', fill.color);
    let fillOpacity = 1;
    if ((style as PathStyleProps).fillOpacity != null) {
      fillOpacity = (style as PathStyleProps).fillOpacity * fill.opacity * opacity;
    } else {
      fillOpacity = fill.opacity * opacity;
    }
    attr(svgEl, 'fill-opacity', `${fillOpacity}`);
  } else {
      attr(svgEl, 'fill', NONE);
  }

  if (pathHasStroke(style)) {
    const stroke = normalizeColor((style as PathStyleProps).stroke as string);
    attr(svgEl, 'stroke', stroke.color);

    const strokeWidth = (style as PathStyleProps).lineWidth;
    const strokeScale = (style as PathStyleProps).strokeNoScale ? (el as Path).getLineScale() : 1;

    attr(svgEl, 'stroke-width', `${strokeScale ? strokeWidth / strokeScale : 0}`);
    attr(svgEl, 'paint-order', (style as PathStyleProps).strokeFirst ? 'stroke' : 'fill');

    let strokeOpacity = 1;
    if ((style as PathStyleProps).strokeOpacity != null) {
      strokeOpacity = (style as PathStyleProps).strokeOpacity * stroke.opacity * opacity;
    } else {
      strokeOpacity = stroke.opacity * opacity;
    }
    attr(svgEl, 'stroke-opacity', `${strokeOpacity}`);

    let lineDash;
    if ((style as PathStyleProps).lineDash && strokeWidth > 0) {
      lineDash = normalizeLineDash((style as PathStyleProps).lineDash, strokeWidth);
    }

    if (lineDash) {
      let lineDashOffset = (style as PathStyleProps).lineDashOffset;
      if (strokeScale && strokeScale !== 1) {
        lineDash = map(lineDash, val => val / strokeScale);
        if (lineDashOffset) {
    ...
    if ((style as PathStyleProps).lineCap) {
      attr(svgEl, 'stroke-lineap', (style as PathStyleProps).lineCap);
    }

    if ((style as PathStyleProps).lineJoin) {
      attr(svgEl, 'stroke-linejoin', (style as PathStyleProps).lineJoin);
    }

    if ((style as PathStyleProps).miterLimit) {
      attr(svgEl, 'stroke-miterlimit', `${(style as PathStyleProps).miterLimit}`);
    }
  } else {
