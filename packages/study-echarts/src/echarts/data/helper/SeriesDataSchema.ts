  ...
  makeStoreSchema(): {
    ...
    for (let fullDimIdx = 0, seriesDimIdx = 0; fullDimIdx < dimCount; fullDimIdx++) {
      ...
      if (seriesDimDef && seriesDimDef.storeDimIndex === fullDimIdx) {
        property = willRetrieveDataByName ? seriesDimDef.name : null as any;
        ...
        if (sourceDimDef) {
          property = willRetrieveDataByName ? sourceDimDef.name : null as any;
          type = sourceDimDef.type!;
        }
      }

      dims.push(({ property: property!, type: type!, ordinalMeta: ordinalMeta! }) as any);
