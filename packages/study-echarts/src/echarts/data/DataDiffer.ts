...
class DataDiffer<CTX = unknown> {
  ...
  constructor(
    ...
    this._oldKeyGetter = (oldKeyGetter || defaultKeyGetter) as any;
    this._newKeyGetter = (newKeyGetter || defaultKeyGetter) as any;
  ...
  private _executeOneToOne(): void {
    ...
    this._initIndexMap(oldArr, null as any, oldDataKeyArr, '_oldKeyGetter');
    ...
    for (let i = 0; i < oldArr.length; i++) {
      ...
      if (newIdxMapValLen > 1) {
        ...
        newDataIndexMap[oldKey] = null as any;
  ...
  private _performRestAdd(newDataKeyArr: string[], newDataIndexMap: DataIndexMap) {
    for (let i = 0; i < newDataKeyArr.length; i++) {
      ...
      newDataIndexMap[newKey] = null as any;
    }
  }
