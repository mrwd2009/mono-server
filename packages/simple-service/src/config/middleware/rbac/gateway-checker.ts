...
export const getUserPermissions = async (id: number): Promise<number[]> => {
  const sql = `
    select rbacrole.__pk_rbacrole, sequence_id from rbacrole inner join rbacuserrole on rbacrole.__pk_rbacrole = rbacuserrole._fk_rbacrole
    where rbacuserrole.app = :appEnv and _fk_user = :userId and enabled = 1
  `;
  ...
  return _.map(permissions, '__pk_rbacpermission');
};
